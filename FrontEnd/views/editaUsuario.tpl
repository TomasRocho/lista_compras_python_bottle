...
<div class="conteudo">
    <form action="/usuario/salvar" method="post">
        ...
        <input type="text" name="dataNascimento"  required value="{{usr['dataNascimento'] if usr else ''}}" placeholder="dd/mm/aaaa" pattern="[0-9]{2}/[0-9]{2}/[0-9]{4}" title="Formato inválido. Use dd/mm/aaaa">
        <label for="email">e-mail:</label>
        <input type="text"  required name="email" value="{{usr['email'] if usr else ''}}">
        ...
        <div style="{{ 'display:none' if not exibeSenha else '' }}">
            ...
            <input type="password" name="senha"  required value="{{usr['senha'] if usr else ''}}">
        </div>
        <br/>
